...
const Boxes = () => {
  ...
  return (
    <div className={cssClasses.boxContainer}>
      {boxData.map((box, i) => {
        return (
          <div key={i} className={`${cssClasses.box} ${cssClasses["box" + i]}`}>
            <div className={cssClasses["boxHeadingContainer" + i]}>
              <Typography
                className={`${classes.bgText} ${cssClasses["bgText" + i]}`}
                variant="h6"
              >
                {box.editBgText ? (
                  <Fragment>
                    {" "}
                    {box.bgText.slice(0, 1)}{" "}
                    <span>{box.bgText.slice(1, 2)}</span>{" "}
                  </Fragment>
                ) : (
                  box.bgText
                )}
